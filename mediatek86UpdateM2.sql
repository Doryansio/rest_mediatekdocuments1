...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : ven. 22 mars 2024 à 01:31
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `majDvd` (`id` VARCHAR(10))   BEGIN
DECLARE nb INTEGER ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `majLivre` (`id` VARCHAR(10))   BEGIN
DECLARE nb INTEGER ;
	SELECT COUNT(*) INTO nb
    FROM dvd
    WHERE id = id.livre ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `majLivreDvd` (`id` VARCHAR(10))   BEGIN
DECLARE nb INTEGER ;
	SELECT COUNT(*)INTO nb
    	FROM revue
    	WHERE id = livres_dvd.id ;
    IF (nb = 1) THEN
    	SIGNAL SQLSTATE "45000"
        	SET MESSAGE_TEXT = " operation impossible";
    END IF; 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `majRevue` (`id` VARCHAR(10))   BEGIN
DECLARE nb INTEGER ;
	SELECT COUNT(*)INTO nb
    	FROM livres_dvd
    	WHERE id = revue.id ;
...
('00007', 'Dans les coulisses du musée         545', '', 'LV001', '00003', '10006'),
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `upDvd`;
DELIMITER $$
CREATE TRIGGER `upDvd` BEFORE INSERT ON `dvd` FOR EACH ROW BEGIN 
	CALL majDvd(NEW.id);
END
$$
DELIMITER ;
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `upLivre`;
DELIMITER $$
CREATE TRIGGER `upLivre` BEFORE INSERT ON `livre` FOR EACH ROW BEGIN
	CALL majLivre(NEW.id);
END
$$
DELIMITER ;
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `upLivreDvd`;
DELIMITER $$
CREATE TRIGGER `upLivreDvd` BEFORE INSERT ON `livres_dvd` FOR EACH ROW BEGIN
	CALL majLivreDvd(NEW.id);
END
$$
DELIMITER ;
